...
export function MobileWorkCard({presentation,onChange,focused=false,onFocus}:{presentation:WorkPresentation;onChange:(value:WorkPresentation)=>void;focused?:boolean;onFocus?:()=>void}) {
  ...
  return <View style={styles.card}>
    ...
    {presentation.integration&&<View accessible accessibilityLabel="Decisão de integração" style={styles.result}>
      <Text style={styles.label}>{presentation.integration.status==='awaiting_decision'?'Integração aguardando sua decisão':presentation.integration.status==='authorized'?'Integração autorizada':presentation.integration.status==='branch_published'?'Branch publicada':'Integração recusada'}</Text>
      <Text style={styles.body}>{presentation.integration.status==='awaiting_decision'?'Autorizar permite uma futura execução protegida; não publica, envia, cria PR ou integra agora.':presentation.integration.status==='authorized'?'Autorizada e aguardando publicação protegida. Nada foi publicado, enviado, integrado ou mergeado.':presentation.integration.status==='branch_published'?`Branch ${presentation.integration.publication?.remoteBranch??''} publicada no commit ${presentation.integration.publication?.commitSha.slice(0,7)??''}. Nenhum PR foi criado e nada foi mergeado ou integrado.`:'A integração deste resultado foi recusada. Nenhum efeito externo ocorreu.'}</Text>
