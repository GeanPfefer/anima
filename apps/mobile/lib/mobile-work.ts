import { WorkOrchestrationService, buildProposalRevision, interpretWorkRequest, isWorkContinuation, presentWorkItem, resolveWorkFocus, type ApprovalDecision, type ResultReviewDecision, type WorkItem, type WorkPresentation, type WorkResultValidation } from '@anima/core';
...
export type MobileWorkRouting={kind:'none'}|{kind:'proposal';presentation:WorkPresentation}|{kind:'continued';workItemId:string}|{kind:'focus_confirmation_required';sourceMessageId:string;candidates:readonly{id:string;summary:string}[]}|{kind:'error';message:string};
