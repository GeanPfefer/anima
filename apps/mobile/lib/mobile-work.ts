...
// UX-01 (paridade mobile) — PEDE pausa/cancelamento de uma tentativa autônoma.
// Só persiste a intenção (request_work_control, RLS); o efeito real é aplicado
// cooperativamente pelo laço num checkpoint seguro. Relê a projeção persistida.
export async function requestWorkControl(value:WorkPresentation,action:'pause'|'cancel'):Promise<WorkPresentation>{const attemptId=value.execution?.attemptId;if(!attemptId)throw new Error('Nenhuma execução autônoma ativa para controlar.');const{error}=await supabase.rpc('request_work_control',{p_work_item_id:value.item.id,p_expected_proposal_version:value.item.proposalVersion,p_attempt_id:attemptId,p_action:action});if(error)throw new Error(error.message);return reloadWork(value.item.id)}
