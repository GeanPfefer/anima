// Executa UMA ÚNICA volta do Resident Host para o successor 5895b59c, já aprovado e classificado.
// A execução paga é governada pela authority humana EXATA (openai / openai-api /
// provider_api:gpt-5.6-terra, 30 min, US$1,50, validade 2026-09-14T23:59:59-03:00, attempts=1).
// Não cria authority adicional, não abre segunda volta, não liquida reserva artificialmente e não
// toca origin/main.
//
// Roteamento forçado para OpenAI SEM RunPod: Router V1 LIGADO + on-demand DESLIGADO + política de
// capacidade local (qwen3-coder exige 20 GiB, capacidade 16 GiB ⇒ local inviável) ⇒ o Router
...
async function main(): Promise<void> {
  ...
  const result = await runProjectBacklogHostTurn({
    client,
    ownerInstanceId: `selfdev-5895b59c-${WORK_ITEM_ID}`,
    maxTurnsPerCycle: 1,
    maxCycles: 1,
