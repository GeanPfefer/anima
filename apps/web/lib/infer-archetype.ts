...
// Atualiza profiles.archetype com base no comportamento recente.
// Chamado fire-and-forget da rota de chat a cada ~15 mensagens.
...
{"explorer": 30, "focused": 25, "builder": 30, "visionary": 15}`;
  ...
  try {
    ...
    if (!keys.every(k => typeof parsed[k] === 'number' && parsed[k]! >= 0)) return;
