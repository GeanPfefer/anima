...
import type { DetectedEntity } from '@/lib/detect-entities';
...
// Diferente de extractEntities (que liga entidade↔atividade via entity_mentions),
// aqui a ligação é direta com o pilar, captando interesses/identidade que não
// geram atividade cronometrada (ex: "amo Nujabes" → pilar Música).
export async function linkEntitiesToPillars(
  ...
  for (const e of entities) {
    ...
    // upsert da entidade por nome (mesmo critério de extract-entities)
    ...
    if (!pillarId) {
      pillarId = (await createPendingPillar(supabase, userId, e.pillarHint)) ?? undefined;
