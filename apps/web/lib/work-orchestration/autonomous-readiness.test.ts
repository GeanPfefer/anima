...
test('projeta a fila autoritativa e explica dependências sem mutation',async()=>{
  const rpc=jest.fn().mockResolvedValue({data:[{work_item_id:one}],error:null});
...
test('falha de leitura da fila permanece fail-closed',async()=>{
  ...
  expect(result.get(one)).toEqual({eligible:false,blockingDependencyIds:[],reason:'not_in_queue'});
});
