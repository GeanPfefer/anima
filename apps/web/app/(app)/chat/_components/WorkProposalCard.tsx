...
import type { ApprovalDecision, ResultReviewDecision, WorkItem, WorkPresentation } from '@anima/core';
...
export function WorkProposalCard({presentation,onChange,focused=false,onFocus}:Props){
  ...
  const [detail,setDetail]=useState('');const[customDeferReason,setCustomDeferReason]=useState('');const[references,setReferences]=useState('');
  ...
  async function mutate(endpoint:string,payload:Record<string,unknown>){if(status!=='idle')return;setStatus('submitting');setError('');const response=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({workItemId:item.id,expectedProposalVersion:item.proposalVersion,...payload})});const body=await response.json().catch(()=>({}));if(response.ok&&body.ok){setMode('none');setDetail('');setReferences('');setCustomDeferReason('');await reload();return;}setError(body.error?.message??'Não foi possível atualizar o trabalho.');setStatus('idle');if(body.error?.code==='version_conflict'||body.error?.code==='ambiguous_outcome')await reload();}
  ...
  return <article className={styles.workCard} aria-label={`Trabalho, versão ${item.proposalVersion}`} aria-busy={busy}>
    ...
    {item.state==='review'&&latestResult&&<section aria-label="Resultado para revisão" className={styles.workNotice}><strong>Resultado · v{latestResult.proposalVersion}</strong><p>{latestResult.summary}</p><dl className={styles.workMeta}><div><dt>Autoria</dt><dd>{latestResult.author}</dd></div><div><dt>Referências</dt><dd>{latestResult.references.length?latestResult.references.join(', '):'Nenhuma referência'}</dd></div></dl></section>}
    ...
    {mode==='result'&&<div className={styles.workDecision}><label>Resumo do resultado<textarea value={detail} onChange={event=>setDetail(event.target.value)}/></label><label>Referências, uma por linha<textarea value={references} onChange={event=>setReferences(event.target.value)}/></label><button disabled={busy||!detail.trim()} onClick={()=>mutate('/api/work-orchestration/results',{result:{summary:detail.trim(),resultReferences:references.split('\n').map(value=>value.trim()).filter(Boolean)}})}>Enviar para revisão</button><button onClick={()=>setMode('none')}>Voltar</button></div>}
