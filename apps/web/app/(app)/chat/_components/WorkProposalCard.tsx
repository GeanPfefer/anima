...
export function WorkProposalCard({presentation,onChange,focused=false,onFocus,autonomousExecutionAllowed,autonomousBlockReason}:Props){
  ...
  const readinessReason=presentation.autonomousReadiness?.reason;
  const projectedBlockReason=autonomousBlockReason??(readinessReason==='blocked_by_dependency'?`Aguardando conclusão de ${presentation.autonomousReadiness?.blockingDependencyIds.join(', ')}`:readinessReason==='work_intelligence_classification_missing'?'A classificação de inteligência desta versão ainda não foi registrada.':readinessReason==='work_intelligence_classification_incomplete'?'A classificação de inteligência desta versão ainda possui eixos desconhecidos.':readinessReason==='not_in_queue'?'O item não integra a fila autônoma autoritativa.':null);
