...
describe('WorkProposalCard', () => {
  ...
  test('inicia somente o trabalho e a versão explícitos no modo autônomo', async () => {
    ...
    await waitFor(()=>expect(global.fetch).toHaveBeenCalledWith('/api/work-orchestration/execution-requests',expect.objectContaining({method:'POST'})));
    const body=JSON.parse((global.fetch as jest.Mock).mock.calls[0][1].body);expect(body).toMatchObject({workItemId:'item',expectedProposalVersion:2});expect(body.requestId).toEqual(expect.any(String));
  });
  test('não oferece execução autônoma quando faltam alvos, permissões ou limites aprovados', () => {
    render(<WorkProposalCard presentation={presentation({item:{...item,state:'approved'},availableActions:['start']})} onChange={jest.fn()} />);
    expect(screen.queryByRole('button',{name:'Executar autonomamente'})).not.toBeInTheDocument();
  });
  test('expõe a preparação sem oferecer execução quando falta classificação', () => {
    ...
    expect(screen.getByText(/classificação de inteligência desta versão ainda não foi registrada/i)).toBeInTheDocument();
  ...
  test('resultado aceito com integração PENDENTE não se declara concluído (coerência fase×integração)',()=>{render(<WorkProposalCard presentation={presentation({item:{...item,state:'completed'},acceptedResult:result,availableActions:[],progress:{phase:'ready_to_integrate',label:'Pronto para integrar',active:false,terminal:false},integration:{status:'awaiting_decision',acceptedResultEventId:'e1',decision:null,availableDecisions:['authorize','refuse']}})} onChange={jest.fn()}/>);
    // A fase ("Pronto para integrar") e a seção de integração pedem uma decisão; a linha
    // de status NÃO pode contradizer afirmando "trabalho concluído".
    expect(screen.getByText(/decisão de integração está pendente/i)).toBeInTheDocument();
    expect(screen.queryByText(/trabalho concluído/i)).toBeNull();
    expect(screen.getByRole('button',{name:'Autorizar integração'})).toBeInTheDocument();
  });
  ...
  test('falha sem retry elegível nunca aparece como sucesso', () => { render(<WorkProposalCard presentation={presentation({item:{...item,state:'failed'},availableActions:[]})} onChange={jest.fn()} />); expect(screen.getByText('A execução falhou; nenhuma nova tentativa está autorizável no estado atual.')).toBeInTheDocument(); expect(screen.queryByText(/concluído/)).not.toBeInTheDocument(); expect(screen.queryByText(/Resultado aceito/)).not.toBeInTheDocument(); });
  test('falha retryable mostra budget e envia um único ato humano de retry',async()=>{const failed=presentation({item:{...item,state:'failed'},availableActions:[],retryReadiness:{status:'RETRY_READY',reason:null,attemptsUsed:1,maxAttempts:2,remainingAttempts:1,sourceAttemptId:'attempt-1',failureEventId:'failure-1',proposalVersion:2}});render(<WorkProposalCard presentation={failed} onChange={jest.fn()}/>);expect(screen.getByText(/1 de 2 tentativas utilizada/)).toBeInTheDocument();fireEvent.click(screen.getByRole('button',{name:'Tentar novamente autonomamente'}));await waitFor(()=>expect(global.fetch).toHaveBeenCalledWith('/api/work-orchestration/retries',expect.objectContaining({method:'POST'})));const body=JSON.parse((global.fetch as jest.Mock).mock.calls[0][1].body);expect(body).toMatchObject({workItemId:'item',expectedProposalVersion:2,failureEventId:'failure-1'});expect(body.retryRequestId).toEqual(expect.any(String));});
  test('falha bloqueada mostra a razão estruturada em vez da mensagem genérica', () => { const failed=presentation({item:{...item,state:'failed'},availableActions:[],retryReadiness:{status:'BLOCKED',reason:'attempt_budget_exhausted',attemptsUsed:2,maxAttempts:2,remainingAttempts:0,sourceAttemptId:null,failureEventId:null,proposalVersion:2}}); render(<WorkProposalCard presentation={failed} onChange={jest.fn()} />); expect(screen.getByText('A execução falhou; nova tentativa indisponível: as tentativas do orçamento se esgotaram.')).toBeInTheDocument(); expect(screen.queryByText(/nenhuma nova tentativa está autorizável/)).not.toBeInTheDocument(); expect(screen.queryByRole('button',{name:'Tentar novamente autonomamente'})).not.toBeInTheDocument(); });
