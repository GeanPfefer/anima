...
export async function POST(request: Request) {
  ...
  const command = await request.json() as CreateWorkProposalCommand;
  const result = await createWorkOrchestrationService(client).createProposal(command);
  console.info('[work-orchestration]', { operation: 'createProposal', sourceMessageId: command.sourceMessageId, result: result.ok ? 'success' : result.error.code });
