...
export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  return Response.json({ ok: true, value: { presentation: {...serializeReconstructedWorkPresentation(item.value,events.value,contexts.value),autonomousReadiness:readiness.get(item.value.id)}, item: serializeWorkItem(item.value), events: events.value.map(serializeWorkEvent), contexts: contexts.value.map(value => ({ ...value, createdAt: value.createdAt.toISOString() })) } });
}
