...
test('cliente desconectado NÃO aborta a operação durável: signal desacoplado do request', async () => {
  // A autorização de integração já está persistida; publicar a branch é efeito
  // mutativo (git push). Abandonar a página / perder a conexão não pode abortar o
  // push no meio (efeito possível + nada persistido = ambiguidade). Mesmo racional
  // de /supervisor-turn e /execute-commanded: o ciclo não herda o lifetime do HTTP.
  createClientMock.mockResolvedValue(authed({ id: 'u' }));
