...
beforeEach(() => { jest.clearAllMocks(); authMock.mockResolvedValue({ client: {}, userId: 'u' }); auditMock.mockResolvedValue([]); });
...
test('autenticado → devolve os registros de auditoria', async () => {
  ...
  expect(await res.json()).toMatchObject({ ok: true, value: [{ nodeId: 'n', orphanRisk: true }] });
});
