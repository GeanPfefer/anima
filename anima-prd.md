...
| revisar | **humana** hoje; existe um **Verifier V0 advisory** read-only (web+mobile) que confere **coerência + consistência com o contrato aprovado** (correlação, escopo, gates, cross-check adversarial) e **NÃO** substitui o humano nem os gates. **Prevenção real de git IMPLEMENTADA (2026-08-15):** a evidência de **arquivos/commit** deixou de depender só da atestação do executor — o host inspeciona a branch descartável e persiste `host_observed_evidence_recorded` (`author=system`/`origin=host`, append-only, idempotente por tentativa); o Verifier compara **observed × attested** e uma mentira sobre arquivos/commit vira `attested_contradicts_observed` (observed vence). Cobertura honesta: **git independente**; **gates ainda atestados** (`coverage.gates=false`). **Parecer versionado IMPLEMENTADO (2026-08-16):** o parecer do Verifier é persistido append-only (`verifier_opinion_recorded`, `author=system`/`origin=verifier`), **versionado** por `(attempt, verifierVersion, base de evidência)` — a chegada da observação ou um Verifier V2 acrescenta um novo parecer sem apagar o anterior; é **advisory e recomputável** (auditoria, não decisão) e **≠ `integration_decided`** (que é `author=user`, pós-aceite). **Investigado (não implementado):** independência de gate exige reexecução pelo host ou mover a execução de gate para fora do contrato do executor — nenhuma trivial (ver [orquestração §Independência dos gates](docs/arquitetura/orquestracao-de-trabalho.md)); a política de maturidade que ponderar a atestação segue futuro/bloqueada | maturidade (evidência acumulando) |
