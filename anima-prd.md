...
**Cobertura do aceite (2026-09-02):** o Verifier v2 cruza os `expectedEffects` da proposal
aprovada com associações explícitas `validation_criteria[].covers` e somente reconhece
cobertura de gates executados com sucesso. Critério aprovado sem evidência torna o parecer
`inconclusive`; o planner é rejeitado fail-closed se materializar N-1 de N critérios. O backlog
canônico preserva e encaminha sua linha `Aceite` ao planejamento. Pareceres v1 históricos não
são reescritos.
...
- [x] **Project Advisor V0 — E2E PASS** — a pergunta sobre estado e próximo passo do Anima desvia antes de qualquer detector/mutação para um `Project Context Builder` allowlisted. O contexto distingue `canonical`, `observed_state`, `evidence` e `historical_record`, inclui observação RLS reduzida de `work_items`/`work_events`, aplica orçamento e redação de segredos e atravessa um boundary OpenAI/Ollama sem ferramentas de repositório. A resposta separa fatos, capacidades comprovadas, fronteiras, direções canônicas, recomendação e limites; claims sem autoridade falham fechados. Prompt/schema/host estão alinhados, OpenAI recebe projeção compatível, Ollama mantém o contrato integral e unicidade permanece no host. **Prova real:** UI→contexto→OpenAI→schema→parser→semântica→apresentação, 20 claims validados, HTTP 200, sem mutação ou workflow operacional.
- [x] **Project Advisor Fresh Operational State V1 — LOCAL PASS** — a observação viva deixou de ser apenas contagens e passou a projetar, no host, metadados RLS de `work_items`, `work_events` e `work_focus` em um snapshot bounded: trabalho ativo, falha ainda não superada na sequência, review, bloqueio, foco e evidência tipada recente. `current_projection`, `event_sequence`, `historical_snapshot` e `canonical` explicitam temporalidade; um claim presente sustentado só por histórico falha fechado. A triagem é advisory, não escreve nem recebe payloads de eventos, pedidos originais ou conteúdo pessoal. Prova local determinística e adversarial concluída; E2E externa ainda requer autorização específica.
- [x] **Project Advisor Fresh Operational State V1 — E2E PASS** — após a tentativa fail-closed e a correção `current_claim_without_live_source`, o reteste pela UI real atravessou snapshot vivo auditável→11 fontes governadas→OpenAI `gpt-5.6-terra`→schema→parser→semantic validator→apresentação HTTP 200. A resposta distinguiu 2 ativos e 13 falhas não superadas (4 visíveis/9 omitidas pelo bound), não inventou bloqueio/review/foco ausentes da parte observada e tratou história sem usá-la como presente. Recomendação permaneceu advisory; banco/Git não mudaram e nenhum workflow foi acionado.
- [x] **Project Advisor Item Drill-down V0 — LOCAL PASS** — perguntas explícitas sobre um item desviam antes do chat mutável, resolvem ID/prefixo/ordinal/foco somente quando inequívocos e pedem esclarecimento em qualquer ambiguidade. Após resolver exatamente um item, o host projeta estado, timeline bounded, tentativa, falha conhecida ou desconhecida, resultado, Verifier e resumos de evidência coder/gate/Git. Pedido, proposta, payload, prompt/output, comando/log, caminho/diff e conteúdo pessoal permanecem fora do contexto. O contrato é puro, temporal, provider-agnostic e read-only; 18 casos focados e as regressões do Advisor/snapshot passaram sem egress. A prova pela UI real ainda exige autorização específica de provider.
- [ ] **Project Advisor Item Drill-down V0 — E2E NOT_PROVEN** — a pergunta autorizada usou o item da fixture remota `d3890a38…`, pertencente a uma identidade descartável diferente da conta autenticada na UI. A resolução user-scoped/RLS recusou corretamente o item com HTTP 404 antes de construir a projeção e antes de chamar OpenAI; nenhuma chamada externa foi consumida e banco/Git permaneceram idênticos. A UI exibiu “Erro desconhecido” porque o 404 era texto puro; a rota agora devolve JSON de erro compatível e coberto. Nova E2E deve usar um item visível à mesma conta da UI (ou autenticar deliberadamente como a identidade-fixture) e exige nova autorização.
- [x] **Project Advisor Item Drill-down V0 — E2E PASS** — o reteste usou o item real `58159655…`, visível sob RLS na mesma conta da UI. A projeção integral de quatro eventos mostrou `in_progress` desde `work_started`, sem tentativa, falha, resultado, Verifier ou evidências coder/gate/Git. Uma única chamada OpenAI `gpt-5.6-terra` atravessou schema, parser e semantic validator (12 claims) e foi apresentada HTTP 200. A resposta preservou cada ausência, não converteu falta de telemetria em falha/diagnóstico e tratou recomendação como advisory. Banco `60/601/2/189`, Git e origens permaneceram idênticos; nenhum workflow/coder foi acionado.
- [x] **Project Advisor Conversational Item Reference V0 — LOCAL PASS** — o servidor extrai somente UUIDs que a resposta validada realmente mencionou, cruza-os com o snapshot e devolve ao cliente UUID/ordinal/papel em metadado efêmero. O cliente mantém apenas o conjunto apresentado mais recente e o devolve no próximo turno; não há tabela, memória paralela ou conteúdo bruto. “primeiro/segundo”, “esse item”, “essa falha”, “por que ele” e referências explícitas resolvem deterministicamente ou pedem esclarecimento. A identidade é contextual, mas estado/eventos são relidos sob RLS no instante do drill-down; referência revogada retorna 404 governado. Provas adversariais locais e gates passaram sem egress ou mutação; E2E externa ainda requer autorização própria.
- [x] **Work Item Presentation / Reencontro V0 — E2E PASS** — o chat deixou de reduzir `sourceMessageId` a um único cartão: a hidratação preserva `0..N` apresentações, com identidade estável por `workItem.id`. A superfície “Trabalhos do projeto” e os cartões detalhados projetam a mesma RPC `autonomous_work_queue()` que governa o Supervisor; ausência da projeção falha fechada e dependências pendentes explicam o bloqueio. Na prova autenticada após refresh, os três siblings coexistiram: Item 1 `approved · v2` elegível com ação autônoma, Itens 2/3 `approved · v2` sem ação autônoma e com a dependência causal exata. Nenhum item, provider ou workflow foi acionado.
- [ ] **Ratified Backlog to Execution V0 — Item 1 aguarda ato humano de retry** — a primeira tentativa permanece `failed/retryable`, append-only, antes do coder. Em 2026-08-24 o contrato local de retry foi fechado: `RETRY_READY` deriva de falha vigente, versão/aprovação/classificação, dependências, ausência de claim/attempt ativa e budget; o ato humano idempotente reabre `approved` sem criar claim/attempt. A UI mostra 1/2 e “Tentar novamente autonomamente”. Execução explícita deixou de rodar no Next: a UI só persiste o sinal e o Resident Host in-process, user-scoped, executa com Governor. Prova controlada create/write/inspect/cleanup de worktree passou. **A attempt 2 real não foi criada**; Itens 2/3 continuam aguardando dependência.
- [ ] **ServedRead Provenance V1 — novo item elegível, execução não iniciada** — o item `26f3c07f…`, `programming/low`, aprovado em v1, recebeu pelo clique humano único a classificação `work_intelligence_classified` seq 38931. A consulta read-only da `autonomous_work_queue()` sob o owner confirmou readiness verdadeira; o item permaneceu `approved`, com zero claim, attempt e pedido autônomo. A preparação corrigida está assim comprovada ponta a ponta. O item estrutural anterior `0898a0c2…` avançou depois do snapshot de 2026-08-25 e hoje está `failed`; sua história é distinta e preservada. Próximo ato: avaliar recursos e, se admitido, pedir execução separadamente pelo Resident Host isolado.
- [x] **Recovery Lineage V0 — substrato persistente + idempotência estrita** — `work_recovery_lineage` e `propose_recovery_successor` já preservavam original `failed`, attempts/budget, owner/RLS e dependências honestas enquanto criavam um successor `proposed`. A reconciliação de 2026-08-26 fechou replay divergente e corrida pela mesma chave: lock transacional por owner+key, equivalência de todos os argumentos governados e conflito `22023` fail-closed. Tipos do Supabase regenerados; pgTAP 16/16. Ainda falta o produtor da aplicação que classifica a falha e formula/materializa o successor governado.
- [x] **Coder local: gate → reparo → revalidação** — Ollama e DeepSeek Harness admitem no máximo um reparo interno no mesmo attempt/worktree; OpenAI permanece com zero. O host fornece gate falho, diagnóstico sanitizado, arquivos alterados e SHA-256 do diff. Falhas ambientais não reparam e diff idêntico encerra sem revalidar. Prova local real em 2026-08-28: `PASS(typecheck) → FAIL(teste) → repair Ollama → PASS → PASS`, terminal result e objetivo semântico atingido. **Prova canônica não concluída:** a attempt 2 do Successor A foi consumida pelo retry RPC + Resident Host, mas o invocador ad hoc iniciou o porto in-process da raiz do monorepo; `projectRoot()` pressupõe o cwd `apps/web` do script npm e resolveu `G:\`, causando `worktree-create-failed` antes do coder. Item 2/2 `failed`; nenhum gate, repair ou Verifier; sem attempt 3.
- [x] **Atualização da prova canônica de repair (2026-08-28)** — `projectRoot` agora descobre somente a raiz Anima entre cwd/ancestrais ou aceita override explícito validado. Recovery successor `f7d50d04…`, attempt `3850dc97…`, mesma worktree: coder editou `work-routing.ts`, gate falhou e um repair foi acionado, mas terminou `ollama_no_effective_edits` antes do re-gate/Verifier. Recovery policy: `human_required/failure_not_classified`; resta 1/2, preservada. Portanto a prova canônica é parcial e a nota anterior de “não concluída antes do coder” permanece apenas como histórico da attempt anterior.
- [x] **No-progress + checkpoint Git durável (2026-08-29)** — `ollama_no_effective_edits` é `no_progress`: tenta novamente uma vez com budget, repetido decompõe e esgotado não vira sucesso. O banco real já continha a attempt 2 `fb79667c…`: edit inicial, mesmo gate falho e novo no-progress; item ficou `failed` 2/2, sem re-gate/Verifier. A reconciliação mostrou que o checkpoint apontava para branch ainda no SHA-base e a worktree fora descartada. Agora o executor commita o checkpoint antes dos gates, compara o estado final sempre ao SHA-base e, se o repair falha, restaura ao commit do checkpoint (não ao base). Assim uma falha continua falha, mas o edit e o handoff permanecem transferíveis. Gates amplos: typecheck dos cinco workspaces, mobile 51, web 1.000, core 1.193, Supabase 9 e build web verdes.
- [x] **Verifier distingue proveniência completa do delta retomado (2026-08-30)** — evidência Git host-side preserva o diff completo contra `baseSha` e registra separadamente o delta contra `startSha`/checkpoint. Scope enforcement usa o delta quando presente; evidência histórica sem delta mantém o comportamento anterior e nenhuma ausência é tratada como lista vazia. Reobservação corrigida e novo parecer são append-only. Prova viva: successor `b811aaa1…`, attempt `f7fd8c2e…`, checkpoint `4b4cf25d`; opinião `rejected` antiga preservada, nova evidência seq 42058 (full implementação+teste; delta só teste) e nova opinião seq 42059 `verified` (7/0/0). Nesse checkpoint o item chegou a `review`; o aceite humano e `completed` estão registrados no item seguinte.
- [x] **Dev Local V1 + review/rework incremental completo (2026-08-30)** — o aceite humano no chat persistiu `result_accepted` seq 42060 sobre o resultado exato `437f7ad9…` e levou o successor `b811aaa1…` a `completed`. A cadeia completa retomou checkpoint, preservou implementação, editou somente o teste restante, reparou/revalidou até gate verde, manteve evidência/opiniões append-only e terminou por decisão humana. Nenhuma integração, publicação, PR, merge ou deploy ocorreu. Ver [Marco 008](docs/marcos/008-dev-local-v1-review-rework-incremental.md).
- [x] **Recovery Decision V0 — falha → estratégia governada** — política pura allowlisted decide `retry | decompose | environment_wait | human_required`; `execution_failed` genérico não autoriza retry, repetição/no-progress/contexto/capacidade do modelo favorecem decomposição, recursos/ambiente esperam mudança e contratos/desconhecido escalam. A aplicação projeta isso read-only de eventos RLS. Prova no Item 1 real: `ollama_read_round_limit`, 2/2 → `decompose`. Ainda não materializa successor automaticamente.
- [x] **Recovery Successor Candidate V0 — decomposição sem expansão de autoridade** — antes da RPC, o domínio exige original falho + assessment correlacionado `decompose`, escopo estritamente menor, mesmo target/capability, permissões/attempts/impacto monotônicos e nenhuma autoridade financeira/dependência no original. O boundary web só cria `proposed` e falha fechado. Core completo: 1.191 testes PASS. A lineage real preexistente foi reconciliada no item seguinte; ainda falta um planejador que formule candidatos sem registro manual.
- [ ] **Item 1 Recovery — fatia mínima existente também falhou** — a reconciliação real encontrou lineage seq 1 já materializada: successor `27c8d1ba…`, aprovado/classificado/executado, falhou em `ollama_read_round_limit` na primeira de duas attempts. A fatia já é produção+teste em uma camada; criar seq 2 igual seria retry disfarçado, não decomposição. Original e successor permanecem `failed`, `satisfies_original_objective=false`, e Item 2 segue bloqueado. Próxima fronteira é decisão de estratégia: protocolo/budget local, backend local alternativo ou provider externo autorizado.
