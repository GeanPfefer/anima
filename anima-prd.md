...
- [ ] **Ratified Backlog to Execution V0 — Item 1 NOT_PROVEN** — o clique real adquiriu claim, persistiu routing e iniciou uma tentativa correlacionada, mas falhou terminalmente antes do coder ao criar a worktree isolada (`checkpoint:worktree-create-failed`). O claim foi liberado, não houve branch/worktree residual, gates/evidências/Verifier não avançaram e Itens 2/3 permaneceram `approved` sem execução. O erro foi marcado retryable pelo executor, porém `failed` é terminal e não existe transição ratificada de retry para essa classe de falha; nenhum retry foi forçado.
