...
- [x] **Work Item Presentation / Reencontro V0 — LOCAL PASS** — o chat deixou de reduzir `sourceMessageId` a um único cartão: a hidratação agora preserva `0..N` apresentações, com identidade estável por `workItem.id`. Uma superfície “Trabalhos do projeto” consulta o GET RLS-scoped existente após refresh, mostra estado, versão, dependências, fonte e readiness, abre o presenter humano existente e mantém a execução autônoma fail-closed para dependências pendentes. A prova foi somente leitura; nenhum item, provider ou workflow foi acionado. A confirmação visual na conta real permanece checkpoint humano antes de declarar a prova E2E completa.
