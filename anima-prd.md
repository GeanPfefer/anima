...
- [ ] **Project Advisor Fresh Operational State V1 — E2E NOT_PROVEN** — a única chamada autorizada em 2026-08-24 atravessou UI, snapshot/contexto governado e OpenAI `gpt-5.6-terra`, mas o semantic validator recusou corretamente `current_claim_without_live_source`: uma fronteira presente veio apoiada apenas por histórico. Sem retry. O contrato local foi alinhado para exigir apoio `current_projection`/`event_sequence` em toda `unprovenFrontier`, mantendo histórico puro disponível no racional como trajetória. Nova prova externa exige nova autorização.
