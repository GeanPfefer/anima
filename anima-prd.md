...
| revisar | **humana** hoje; existe um **Verifier V0 advisory** read-only (web+mobile) que confere **coerência + consistência com o contrato aprovado** (correlação, escopo, gates, cross-check adversarial) e **NÃO** substitui o humano nem os gates. Descoberta auditada: a evidência de gate/arquivo é **atestada pelo executor**, não re-observada — o parecer marca isso (`restsOnAttestedEvidence`) e um executor cuidadoso ainda fabricaria um `verified`. A **prevenção real** (evento `system` de gate/diff observado pelo host) e a política de maturidade que ponderar a atestação seguem futuro | maturidade (evidência acumulando) |
