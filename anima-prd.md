...
> Documento vivo de design. Última atualização: 2026-08-25 (sessão: reconciliação da classificação autônoma ausente separada da execução; readiness passa a expor o motivo real e oferece preparação autenticada sem claim/attempt; ver [registro](docs/registros/2026-08-25-readiness-classification-reconciliation.md) e [Plano 002](docs/planos/002-modo-autonomo-v0.md))
...
- [ ] **ServedRead Provenance V1 — classificação pendente, execução não iniciada** — o item real `0898a0c2…` voltou canonicamente a `approved` após `manual_work_released`, mas a fila o exclui porque não existe `work_intelligence_classified` vigente para a v3. AUTO-01, aprovação, dependências e ausência de claim/attempt estão válidos; `base_sha=ecf4e07…` não é a guarda bloqueadora. A UI agora expõe `work_intelligence_classification_missing` e oferece preparação autenticada, append-only e sem execução. Próximo ato: o usuário correto aciona “Preparar elegibilidade autônoma”, relê a fila e revisa os botões; coder permanece proibido até esse ato.
