...
> Documento vivo de design. Última atualização: 2026-08-25 (sessão: preparação de classificação corrigida para propostas estruturais já aprovadas; erro real e replay reconciliado passam pelo HTTP sem claim/attempt; ver [registro](docs/registros/2026-08-25-preparacao-classificacao-estrutural.md) e [Plano 002](docs/planos/002-modo-autonomo-v0.md))
...
- [ ] **ServedRead Provenance V1 — classificação pendente, execução não iniciada** — o primeiro clique em “Preparar elegibilidade autônoma” não persistiu nada: a rota recusou `impact_level=structural` pela ponte restrita a `low` e devolveu 409 sem mensagem. A preparação agora admite somente `low` ou `structural` dentro do restante do envelope planejado/aprovado; estrutural é classificado conservadoramente como `bounded/moderate/conditionally_reversible/clear/normal`. Escrita ambígua é relida e tratada como replay se o fato corrente existir. O item real permanece `approved` v3, sem classification/claim/attempt/request. Próximo ato continua humano: clicar uma vez em “Preparar elegibilidade autônoma”; não iniciar o coder.
