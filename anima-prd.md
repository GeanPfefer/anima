...
- [x] **Work Item Presentation / Reencontro V0 — LOCAL PASS** — o chat deixou de reduzir `sourceMessageId` a um único cartão: a hidratação agora preserva `0..N` apresentações, com identidade estável por `workItem.id`. Uma superfície “Trabalhos do projeto” consulta o GET RLS-scoped existente após refresh, mostra estado, versão, dependências, fonte e readiness e abre o presenter humano existente. Após a prova visual revelar divergência nos cartões reidratados, lista, agrupamento e detalhe passaram a projetar a mesma RPC `autonomous_work_queue()` que governa o Supervisor; ausência da projeção falha fechada e dependências pendentes explicam o bloqueio. Nenhum item, provider ou workflow foi acionado. A confirmação visual final na conta real permanece checkpoint humano antes de declarar a prova E2E completa.
