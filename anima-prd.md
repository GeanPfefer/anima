...
> Documento vivo de design. Última atualização: 2026-08-26 (sessão: candidato de decomposição governado ligado ao boundary de successor, com autoridade monotônica; ver [registro](docs/registros/2026-08-26-candidato-governado-de-recovery.md) e [Plano 002](docs/planos/002-modo-autonomo-v0.md))
...
- [x] **Recovery Successor Candidate V0 — decomposição sem expansão de autoridade** — antes da RPC, o domínio exige original falho + assessment correlacionado `decompose`, escopo estritamente menor, mesmo target/capability, permissões/attempts/impacto monotônicos e nenhuma autoridade financeira/dependência no original. O boundary web só cria `proposed` e falha fechado. Core completo: 1.191 testes PASS. Falta a materialização real da fatia do Item 1 e, depois, um planejador que a formule sem registro manual.
