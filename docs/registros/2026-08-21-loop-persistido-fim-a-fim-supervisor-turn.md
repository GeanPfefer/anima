...
## Addendum (mesma sessão) — auth CANÔNICO emitido pelo GoTrue (3ª rodada)
As duas primeiras rodadas usaram um token JWT assinado localmente com o GoTrue secret — legítimo (o `authenticateRequest` valida via `getUser(token)` do GoTrue real, que aceitou a assinatura + carregou o usuário), mas não obtido por um fluxo de login. Para remover essa ressalva, uma 3ª rodada usou um **access_token EMITIDO pelo próprio GoTrue** pelo fluxo admin passwordless (BOOTSTRAP-DA-PROVA, sem mutar a conta, sem senha no repo): `POST /auth/v1/admin/generate_link` (magiclink) → `POST /auth/v1/verify` (token_hash) → `access_token`; confirmado por `GET /auth/v1/user` (sub=usuário dev, role authenticated). O restante (seed via RPCs como o usuário; POST supervisor-turn; execução; persistência) idêntico e real.
- **Attempt `c944e67b`** (item `93fe6a89`): cadeia idêntica e completa, **verdict `verified`**, `coverage {git:true,gates:true}`, findings `scope/gates_independently_observed` (independent), git `["packages/core/src/xp.ts"]`, gate `npm.cmd test --workspace=packages/core passed exit0`, item `review`, diff real na branch (`+// anima-canonical-proof-…`), workspace real intacto. Timestamps do banco mostram a ordem real: approve 14:16:20 → classified 14:17:02 → checkpoint 14:17:41 → result 14:18:17 → gate/coder/git/verifier 14:18:17–18 (pós-result, nesta ordem, fail-open).
- **3/3 rodadas persistidas verdes** no total (`bd5dffe4`, `c10a1a26`, `c944e67b`). O token canônico foi obtido pelo endpoint real do GoTrue; `authenticateRequest`→`getUser` é o mesmo validador em produção.
