...
**Estado (2026-07-31): implementado e pronto para revisão (não ratificado).** Lacuna: a hidratação do chat reconstruía cartões só da sessão ativa (`items/by-source`), então o trabalho aberto de uma conversa arquivada ficava inalcançável em outra. UX-04 estendeu o existente sem replicar domínio: `RESUMABLE_WORK_STATES` + `isWorkHistoryQuery` (intenção determinística de reencontrar/listar), `findResumableWorkItems()` (itens não terminais do usuário, isolados por RLS, ordem determinística), `GET /api/work-orchestration/items` (mesma projeção reconstruída dos cartões), intenção `work_history` na rota do chat e renderização da lista no `ChatClient` reusando `WorkProposalCard` com todas as ações. Terminais/decididos/aceitos reaparecem só como histórico (projeção existente garante ausência de ação repetível); a lista exclui terminais; retomada continua separada de integração. Sem schema novo — isolamento pela RLS `SELECT` já existente de `work_items`. Verde: typecheck; core 588, web 132, mobile 12. Prova integrada contra os endpoints reais do Next.js com conta descartável (a automação de navegador não pôde dirigir os inputs/forms nesta sessão, então a sessão veio do GoTrue e as etapas por HTTP autenticado; a renderização está coberta pelo teste de componente): item criado pela rota real → `review`/`changes_requested` na conversa #1 → conversa arquivada → em nova conversa `GET /items` e `work_history` **reconstruíram os itens do histórico através da sessão arquivada** → retomada (aceite) → reload já **exclui** o terminal → 2ª conta não vê nada (RLS). Descartáveis removidos por identificação exata; registros, contagens e dados protegidos verificados retornaram ao baseline observado (sem comparação binária); `tecopfefer@gmail.com` intacto. Detalhes em "UX-04 pronto para revisão (2026-07-31)" no [Plano 002](002-modo-autonomo-v0.md). Fora: paridade mobile; dashboard/timeline. Definição original abaixo.
