...
9. [x] **INT-05 — handoff durável de worktree, fiado ao produtor vivo** — contrato puro `worktree-handoff.ts` em `packages/core` (`buildWorktreeHandoff`/`parseWorktreeHandoff`/`projectWorktreeHandoff`), 21 testes do módulo, na branch `claude/worktree-durable-handoff`. Estrutura a evidência git que sobrevive à remoção da worktree (base/commit SHA, branch do namespace `anima-work/`, resumo do diff em contagens, gates e desfecho, erro seguro), com validação fail-closed (versão, correlação, SHA, namespace, status↔gates, limites de tamanho, segredos/caminhos absolutos). **Produtor vivo fiado:** o `WorktreeExecutorAdapter` coleta gates e o diff estruturado (`GitWorktree.diffNumstat`), captura o `commitSha` real da branch descartável e embute o `worktreeHandoff` no sinal `result` (`buildWorktreeHandoff`); o contrato `WorkExecutorSignal.result` ganhou o campo **opcional** `worktreeHandoff`, e `validateWorkExecutorTranscript` o valida **fail-closed** quando presente (re-parse + correlação da tríade) e **aceita a ausência** (outros executores não o emitem). O sinal terminal inteiro é persistido em `data.executor_signal` pela RPC de término (`record_commanded_work_terminal` grava o sinal como JSONB genérico) e relido por `projectWorktreeHandoff` — **sem migration**. Determinístico: **sem `createdAt` nem relógio de parede** no módulo (o instante durável vem do envelope do evento persistido) e ordenação canônica de arquivos/gates, de modo que a mesma evidência lógica produz estrutura byte-idêntica e a reentrega do mesmo sinal terminal é idempotente. O produtor embute o handoff **apenas no desfecho de sucesso** (`result`/`review`); gate reprovado continua saindo como sinal `error`, sem o handoff estruturado. `projectWorktreeHandoff` **ainda não tem consumidor vivo** (só testes) — a projeção existe de propósito para o INT-03 consumir. Módulo puro: não aplica, não faz merge, não faz push; o adaptador nunca faz push/merge/apply e a branch descartável é preservada apenas como referência revisável. Provas: 21 (handoff) + 34 (contrato do sinal) + 15 (adaptador de worktree), com core 613 e web work-orchestration verdes e typecheck do monorepo limpo.
10. [ ] **Camada de aplicação/integração/publicação real (persistência + mecânica Git)** — o **contrato puro do INT-03** (`IntegrationBoundary` em `integration-boundary.ts`: `result_produced → result_accepted → integration_authorized/refused → integrated`, com a **segunda aprovação humana** em `decideIntegration({decision:'authorize', origin:'user'})`, distinta do aceite do resultado) **já está concluído e ratificado por Gean (2026-07-20)** — ver o item INT-03 no [backlog do Plano 002](../planos/002-modo-autonomo-v0-backlog.md) e a seção *Execução separada de integração (INT-03, Fase C)* em [orquestração de trabalho](orquestracao-de-trabalho.md). Permanecem **abertos e não iniciados**, e o próprio INT-03 os declara "itens posteriores": (a) a **persistência** dessa fronteira (migration/RPC/pgTAP) — hoje **inexistente**; e (b) a **mecânica real** de publicar/mergear/aplicar (branch/PR/deploy). Essa camada **consome** tanto o contrato ratificado do INT-03 quanto a evidência durável do item 9 (`projectWorktreeHandoff`, hoje **sem chamador vivo**); nenhum efeito Git externo pode ocorrer sem a segunda aprovação humana já modelada. Nada aqui foi começado.

> **Nota de nomenclatura (reconciliada 2026-08-09).** O backlog canônico do Plano 002 define a série **INT-01..INT-04**; **não existe "INT-05" formal**. Neste ADR, **"INT-05" é o rótulo de trabalho** do *substrato de handoff durável de worktree* (item 9), consistente com os commits da branch `claude/worktree-durable-handoff`. **"INT-03"** é, canonicamente, o *contrato puro de fronteira de integração* (`IntegrationBoundary`), **já ratificado** — e por definição **não** inclui persistência nem mecânica real de Git/PR/publicação, que ele mesmo delega a itens posteriores (item 10 acima). Referências antigas a "INT-03" como sinônimo da camada de aplicação real são imprecisas e ficam **superadas** por esta nota; a fronteira aberta é a do item 10, sem ID formal de backlog.

## Protocolo limitado do backend Ollama (2026-08-06)

**Problema comprovado ao vivo:** o `OllamaCoderBackend` original injetava o conteúdo **integral** dos arquivos do escopo no prompt e exigia o conteúdo **integral** de volta em JSON. Numa execução real de reconciliação documental, o prompt de 4 docs (~73k tokens) foi **truncado** pelo `num_ctx=8192` (o modelo avaliou só ~4098 tokens: `prompt_eval_count=4098`), o system prompt com o schema se perdeu, e o modelo devolveu um JSON válido de **schema errado** (`{"status":"completed",…}`), recusado como "0 arquivos". Confirmado que reemitir um único doc de 17 KB com `num_ctx=32768` levou >300 s e falhou — **round-trip de conteúdo integral é inviável**, não é questão de escopo.

**Decisão:** substituir o round-trip integral por um **protocolo limitado em duas fases** (`ollama-protocol.ts`), sem crescer a janela sem teto.

- **Fase 1 — leitura limitada.** O modelo recebe apenas um **manifesto** (caminho, tamanho, linhas, `sha256`, estrutura resumida: headings de Markdown / assinaturas exportadas de TS), nunca o conteúdo. Pede **trechos** por busca, intervalo de linhas ou início, sempre numerados e limitados (linhas, caracteres, quantidade), por no máximo um número pequeno de **rodadas** (padrão 3). Cada trecho vem com o `sha256` atual do arquivo.
- **Fase 2 — edições exatas.** O modelo devolve **operações estruturadas**, não arquivos: `replace_exact` (`path`, `expected_file_sha256`, `before`, `after`, `expected_occurrences=1`) e `create_file` (arquivo novo do escopo). O host valida escopo, `sha256`, ocorrência única, não-sobreposição e mudança real, e aplica **só na worktree** — nunca push/PR/merge. Exclusão não existe neste recorte.

**Orçamento e observabilidade:** `num_ctx` = min(limite declarado, teto operacional conservador), com reserva explícita de saída e `num_predict` limitados; estimativa de tokens **antes** da chamada (recusa se exceder) e detecção de **truncamento** pelos metadados do Ollama **depois**; timeout explícito. Erros carregam código específico (`ollama_context_budget_exceeded`, `ollama_prompt_truncated`, `ollama_invalid_response_schema`, `ollama_read_round_limit`, `ollama_edit_outside_scope`, `ollama_stale_file_hash`, `ollama_ambiguous_replacement`, `ollama_no_effective_edits`, timeout/transporte) — nunca o genérico "nenhum arquivo válido". Nenhum log grava conteúdo integral ou segredos.

**Compatibilidade:** o `parseScopedFiles` genérico **não** foi afrouxado; o `ScriptedCoderBackend` (fake determinístico) e o `GptCoderBackend` seguem intactos; o `WorktreeExecutorAdapter` continua recebendo o conjunto final de arquivos escritos na worktree, sem mudança de schema no banco. Módulos: `ollama-protocol.ts` (puro) + `ollama-coder.ts` (fiação). Commits na branch `claude/worktree-durable-handoff`; sem push/PR/merge.

**Outcome atomicity do lote de edição (restore-to-base):** a garantia é de **desfecho** (outcome atomicity), **não** de atomicidade de **visibilidade** — durante escritas sequenciais pode existir estado intermediário parcial em disco na worktree, mas nada o observa (os gates rodam só após a aplicação concluir). A propriedade: a worktree parte de um estado-base conhecido (checkout do SHA); um lote só é considerado aplicado se **todas** as escritas concluírem; se qualquer escrita falhar, a **camada da worktree** restaura o estado-base **antes** de prosseguir com o tratamento da falha; nenhum estado parcial pode ser commitado, submetido para revisão ou propagado ao workspace original/`origin/main`.

- **Autoridade única de restauração:** a validação integral contra o snapshot continua **antes** da primeira escrita (`applyEditOperations`); a escrita (`writeChangeSet`) apenas grava-ou-lança e **nunca** retorna sucesso parcial; a restauração é **exclusivamente** da `GitWorktree.restoreToBase()` (o backend não conhece Git e não faz rollback próprio — uma autoridade, não duas).
- **`restoreToBase()`:** desfaz o link de `node_modules` primeiro (para o `clean` nunca recursar pelo junction), `git reset --hard <SHA-base>` (rastreados) e `git clean -fdx` (não-rastreados **e ignorados** — o `-x` remove também `create_file` sob `.gitignore`). Nesta fase a worktree só contém as edições do backend; nenhuma infra ignorada existe ainda. `create_file` volta a ser suportado, com rollback comprovado. **Não recebe o AbortSignal da tentativa, de propósito:** cleanup preserva o invariant e não é trabalho produtivo, então o cancelamento da tentativa **jamais cancela a restauração** (senão o `runProcess` mataria um `reset`/`clean` cujo signal já estivesse abortado). Contrato de erro total: **nunca rejeita** — devolve `false` em qualquer falha.
- **Falha da própria restauração:** `restoreToBase()` devolve `false`; o executor registra a falha de restauração e mantém a worktree **condenada ao `dispose`** — jamais converte em sucesso.
- **`dispose` é a contenção final:** qualquer worktree de tentativa é removida ao fim; nenhum gate roda após uma aplicação que falhou e nenhum commit ocorre antes do lote completo + gates verdes. Toca `GitWorktree` e `WorktreeExecutorAdapter` no mínimo; `CoderWorkspace` e demais contratos permanecem intactos.
