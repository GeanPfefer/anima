...
**Atomicidade do lote de edição (`applyChangesAtomically`):** um lote nunca deixa aplicação parcial na worktree. Toda operação é validada contra o snapshot original **antes** da primeira escrita; ou o lote inteiro é gravado, ou o que já foi escrito é **revertido** ao snapshot (o rollback de `replace_exact` reescreve o conteúdo original, restaurando os bytes exatos). `abort`, `writeFile===false` ou exceção durante a aplicação sempre revertem e lançam — nunca retornam sucesso parcial. **Limitação conhecida:** `create_file` não participa da atomicidade porque revertê-lo exigiria remoção de arquivo, capacidade que o `CoderWorkspace` (só `readFile`/`writeFile`) não expõe; por isso um lote que contenha `create_file` é **recusado fail-closed antes de qualquer escrita**. **Menor ajuste** para habilitá-lo com atomicidade real (commit próprio, sob ratificação): expor um `removeFile` no `CoderWorkspace`, implementado pela `GitWorktree` com o `unlink`+`safeJoin` que ela já possui, e incluir os criados no rollback.
