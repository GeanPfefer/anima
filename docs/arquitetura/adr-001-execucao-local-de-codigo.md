...
10. [~] **Camada de aplicação/integração/publicação real.** A decisão humana e a publicação protegida da branch estão persistidas e implementadas; `projectWorktreeHandoff` possui consumidor vivo no coordenador fail-closed. Criação real de review request, merge/apply/deploy e `integrated` permanecem abertos e exigem nova autorização. Ver [ADR-002](adr-002-integracao-aplicacao-publicacao.md).
