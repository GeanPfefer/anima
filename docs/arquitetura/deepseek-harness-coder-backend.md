...
### Observabilidade sanitizada — ENTREGUE nesta sessão (commit `ee63387`)

O recorte de observabilidade acima passou de "próximo" a feito. O spawner captura só stderr (stdout do modelo segue ignorado), limitado a 8KB e drenado, mais o erro de spawn (ENOENT). `summarizeHarnessFailure`/`redactHarnessPaths` (puros) resumem a falha redigindo caminhos absolutos e limitando o tamanho, preferindo a última linha "de erro" ao ruído final do stack do Node; só um turno de **erro** carrega `diagnostic`, incluído antes da descrição do turno ("exit 134: …" > "kind=error"). Fronteira mantida: evidência (stderr bruto) fica só no host, efêmera; nada de transcript/segredo em `work_events`. Provas: harness/executor **106/106**, typecheck limpo, verificação viva (falha real do DSH → stack `MODULE_NOT_FOUND` capturado e redigido) e **prova viva final** do fluxo real com todas as mudanças = `Verifier verified`. **Próximo:** planner local (custo), sem tornar o Harness default.
