-- Aceita (e valida) o campo OPCIONAL `providerRef` na evidência de lifecycle de node.
-- Retrocompatível: evidência SEM o campo (`? 'providerRef'` falso) e com `null` permanecem
-- válidas; quando presente e não-nula, precisa ser string não-vazia. NUNCA é credencial (o
-- núcleo já rejeita dado sensível no build). Não altera a identidade semântica (o índice único
-- continua item+node+lease+attempt+transição); só o validador ganha a checagem.
...
RETURNS boolean LANGUAGE sql IMMUTABLE SET search_path = pg_catalog AS $$
  SELECT CASE
    ...
    WHEN p ? 'providerRef' AND p->'providerRef' <> 'null'::jsonb AND NOT private.jsonb_is_nonblank_string(p->'providerRef') THEN false
