-- Persiste somente um receipt de branch já verificada pelo provider. Não faz push,
-- não cria PR, não muda o estado do item e não registra integrated.
...
CREATE FUNCTION public.record_branch_published(work_item_id uuid,expected_proposal_version integer,authorization_decision_id text,receipt jsonb)
...
BEGIN
 ...
 IF expected_proposal_version<1 OR length(btrim(coalesce(authorization_decision_id,'')))=0 OR jsonb_typeof(receipt)<>'object' THEN RAISE EXCEPTION 'invalid branch publication input' USING ERRCODE='22023'; END IF;
 ...
 IF receipt->>'kind'<>'branch_publication' OR receipt->>'idempotencyKey'<>('integration-publication:'||authorization_decision_id||':'||(v_handoff->>'commitSha')||':branch')
   OR receipt->>'remoteBranch' IS DISTINCT FROM v_handoff->>'branch' OR receipt->>'commitSha' IS DISTINCT FROM v_handoff->>'commitSha'
   OR receipt->>'verifiedBaseSha' IS DISTINCT FROM v_handoff->>'baseSha'
   OR length(btrim(coalesce(receipt->>'receiptId','')))=0 OR length(btrim(coalesce(receipt->>'providerId','')))=0
   OR length(btrim(coalesce(receipt->>'repositoryId','')))=0 OR length(btrim(coalesce(receipt->>'remoteName','')))=0
   OR length(btrim(coalesce(receipt->>'baseBranch','')))=0
   OR receipt->>'disposition' NOT IN ('created','already_existed') OR v_handoff->>'workItemId'<>v_item.id::text
   OR v_handoff->>'attemptId' IS DISTINCT FROM v_decision.payload->'data'->>'attempt_id' OR (v_handoff->>'approvedProposalVersion')::integer<>expected_proposal_version
 ...
 IF FOUND THEN IF v_existing.payload->'data'->'receipt'=receipt THEN RETURN jsonb_build_object('action','replayed','event_seq',v_existing.seq); END IF;RAISE EXCEPTION 'branch publication receipt conflict' USING ERRCODE='55000'; END IF;
...
END $$;
REVOKE ALL ON FUNCTION public.record_branch_published(uuid,integer,text,jsonb) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.record_branch_published(uuid,integer,text,jsonb) TO authenticated,service_role;
