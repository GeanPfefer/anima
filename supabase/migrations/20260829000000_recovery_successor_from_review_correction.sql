...
--
-- A infraestrutura de sucessor de recuperação (record_recovery_successor) já
-- materializa, idempotente e com lineage, a MENOR unidade sucessora `proposed`
-- que retoma de um checkpoint durável reduzindo o escopo — sem ampliar o
-- envelope. Até aqui ela só aceitava um original FALHO (decomposição de falha).
--
-- Esta migração ADITIVA relaxa APENAS o precondition de estado para aceitar
-- também um original em `changes_requested`: quando a revisão humana pode ser
-- satisfeita por um subconjunto ESTRITO do escopo (ex.: adicionar testes) e
-- existe checkpoint durável elegível, refazer tudo desde a base reescreveria
-- trabalho já verificado. O sucessor retoma do checkpoint e reduz o escopo ao
-- restante, preservando a implementação (fora do escopo). Nada mais muda: a
-- validação de envelope (host), a idempotência estrita, o boundary máximo
-- `proposed`, a lineage e a exigência de aprovação humana permanecem intactas.
-- Backward-compatible: um original `failed` continua aceito exatamente como antes.

CREATE OR REPLACE FUNCTION private.record_recovery_successor(p_user_id uuid, p_original_work_item_id uuid, p_recovery_sequence integer, p_impact_level work_impact_level, p_capability work_capability, p_intent jsonb, p_proposal jsonb, p_recovery_reason text, p_idempotency_key uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  IF v_original.state NOT IN ('failed', 'changes_requested') THEN
    RAISE EXCEPTION 'recovery successor requires a failed or changes_requested original' USING ERRCODE = '55000';
  END IF;
