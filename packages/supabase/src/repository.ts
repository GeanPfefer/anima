...
export class SupabaseWorkOrchestrationRepository implements WorkOrchestrationRepository {
  ...
  createProposal(c:CreateWorkProposalCommand){return this.mutate('create_work_proposal',{source_message_id:c.sourceMessageId,impact_level:c.impactLevel,capability:c.capability,intent:c.intent as Json,proposal:proposalToDatabase(c.proposal)});}
  reviseProposal(c:ReviseWorkProposalCommand){return this.mutate('revise_work_proposal',{work_item_id:c.workItemId,expected_proposal_version:c.expectedProposalVersion,intent:c.intent as Json,proposal:proposalToDatabase(c.proposal)},c);}
  ...
  private async mutate<N extends MutationName>(name:N,args:Database['public']['Functions'][N]['Args'],observed?:ObservedVersion):Promise<WorkOperationResult<WorkItem>>{const{data,error}=await this.client.rpc(name,args);if(error){const mapped=mapSupabaseFailure<WorkItem>(error,true);if(!mapped.ok&&mapped.error.code==='ambiguous_outcome'&&observed){const current=await this.getItem(observed.workItemId);if(current.ok&&current.value.proposalVersion!==observed.expectedProposalVersion)return failure('version_conflict','O item mudou desde a última leitura.');}return mapped;}if(!data)return failure('ambiguous_outcome','A operação não retornou confirmação. Reconcilie antes de repetir.');try{return{ok:true,value:mapWorkItem(data as unknown as Tables<'work_items'>)}}catch(cause){return failure('persistence_failure','Resposta persistida inválida.',false,cause)}}
}
