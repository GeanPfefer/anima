...
describe('adapter Supabase',()=>{
  ...
  test('converte revisão sem permitir estado ou evento',async()=>{const rpc=jest.fn().mockResolvedValue({data:{...row,state:'completed'},error:null});const repo=new SupabaseWorkOrchestrationRepository(clientWithRpc(rpc));await repo.reviewResult({workItemId:'i',expectedProposalVersion:1,reviewedResultEventId:'e1',decision:{type:'accept'}});expect(rpc).toHaveBeenCalledWith('review_work_result_versioned',{work_item_id:'i',expected_proposal_version:1,reviewed_result_event_id:'e1',decision:'accept',decision_context:{}});});
